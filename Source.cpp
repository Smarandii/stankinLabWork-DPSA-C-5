...
// реализовать две сортировки сортировка вставками, сортировка выбора, сортировка обмена
...
void insertion_sort(int unsorted_array[10]) {
	...
	for (int i = 1; i < 10; i++) {
		...
		{
			if (unsorted_array[j] <= x) { sorted_lenght++; unsorted_array[j + 1] = x; print_an_array(unsorted_array, sorted_lenght); break; }
			if (unsorted_array[j] >= x) { unsorted_array[j + 1] = unsorted_array[j]; print_an_array(unsorted_array, sorted_lenght);}
			if (j == 0 && unsorted_array[j] >= x) { sorted_lenght++; unsorted_array[j] = x; print_an_array(unsorted_array, sorted_lenght); break;}
